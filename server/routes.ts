...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Add a special middleware to handle the message retrieval endpoint
  app.use('/api/message/:conversationId/:messageId', async (req, res, next) => {
    // Only process if this is a GET request
    if (req.method === 'GET') {
      // Explicitly set content type to avoid Vite catching this
      ...
      try {
        ...
        // Send JSON response and end the request
    ...
    // If not GET, pass to next middleware
    next();
  });
